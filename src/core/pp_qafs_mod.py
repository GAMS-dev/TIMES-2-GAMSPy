...
class PpQafsMod(GamsClass):
    """Translation unit for pp_qafs.mod."""

    # Instance attributes
    module_name: str = "pp_qafs_mod"
    gams_source: str = "pp_qafs.mod"
    ...
    def compile(self) -> None:
        g = self.tc
        m = g.container
        r, allyear, p, cg, io, c = g.r, g.allyear, g.p, g.cg, g.io, g.c
        g.Rtpgig = Set(m, name="RTPGIG", domain=[r, allyear, p, cg, io, cg])
        g.RtpCg = Set(m, name="RTP_CG", domain=[r, allyear, p, cg, io])
        g.RtpGrp = Set(m, name="RTP_GRP", domain=[r, allyear, p, cg, io])
        g.RpGic = Set(m, name="RP_GIC", domain=[r, p, cg, io, c])

        self.tc.enqueue(self.pp_qafs_exec1, rl=self.env.rl, pl=self.env.pl)
        if not self.env.is_set("sharetol"):
            self.env.set_local("sharetol", "9E-5")
        self.tc.enqueue(
            self.pp_qafs_exec2,
            rl=self.env.rl,
            pl=self.env.pl,
            sharetol=self.env.sharetol,
            notsharerelaxno=(self.env.sharelax != "NO"),
        )

    def pp_qafs_exec1(self: PpQafsMod, rl: str, pl: str) -> None:
...
{pp_qaput("PUTOUT", "PUTGRP", "07", "Unsupported FLO_SHARE: C not in RPC or CG")}
...
{pp_qaput("PUTOUT", "PUTGRP", "01", "FLO_SHAR conflict: Both FX + LO/UP specified, latter ignored")}
...
* Get the good Groups by including those that have not been fixed
 ...
 LOOP(RTP_CG(R,V,P,CG,IO)$(NOT RTP_GRP(R,V,P,CG,IO)),
   ...
   F = MOD(Z,8);
   IF(ABS(F-1) GT .001,
{pp_qaput("PUTOUT", "PUTGRP", "07", "Inconsistent sum of fixed FLO_SHAREs in Group")}
     PUT QLOG ' SEVERE ERROR  -     R=',{rl},' P=',{pl},' V=',V.TL,' CG=',CG.TL,' SUM=',F:0:5;);
   F = MAX(F-1,F/Z*8-{sharetol});
{"LOOP(RP_GIC(R,P,CG,IO,C)$Z, IF(FLO_SHAR(R,V,P,C,CG,'ANNUAL','FX') GT F, Z=0; RTPGIG(R,V,P,CG,IO,C) = YES));" if notsharerelaxno else ""}
 );
 IF(CARD(RTPGIG),
...
{"LOOP(RP_GIC(R,P,CG,IO,C)$Z, MY_F=FLO_SHAR(R,V,P,C,CG,'ANNUAL','UP'); IF((MY_F GT F-1)$MY_F, Z=0; RTPGIG(R,V,P,CG,'OUT',C)=YES));" if notsharerelaxno else ""}
     IF(F LT 1-{sharetol}$(NOT Z),
{pp_qaput("PUTOUT", "PUTGRP", "07", "Defective sum of FX and UP FLO_SHAREs in Group")}
      PUT QLOG ' SEVERE ERROR  -     R=',{rl},' P=',{pl},' V=',V.TL,' CG=',CG.TL,' SUM=',F:0:5;
...
{pp_qaput("PUTOUT", "PUTGRP", "07", "Excessive sum of FX and LO FLO_SHAREs in Group")}
      PUT QLOG ' SEVERE ERROR  -     R=',{rl},' P=',{pl},' V=',V.TL,' CG=',CG.TL,' SUM=',F:0:5;);
      F = MAX(0,F-1); LOOP((RP_GIC(R,P,CG,IO,C),BDLOX)$Z, IF(FLO_SHAR(R,V,P,C,CG,'ANNUAL',BDLOX) GT F, Z=0;
{"RTPGIG(R,V,P,CG,'OUT',C) = YES; IF(MY_F,PUT QLOG ' (Auto-relaxed)';);" if notsharerelaxno else ""}
   ))));
  OPTION CLEAR=RTP_GRP,CLEAR=RTP_CGC;
  LOOP(IO,LOOP(RTPGIG(R,V,P,CG,'OUT',C)$RP_GIC(R,P,CG,IO,C),RTP_GRP(R,V,P,CG,IO) = YES);
    RTP_CGC(R,V,P,CG,C)$(RP_GIC(R,P,CG,IO,C)$RTP_GRP(R,V,P,CG,IO)) = YES);
